...
build, only **AML** is `active=True` and wired into live cycles end-to-end
-- its MeSH heading and full Tier 1 + Tier 2-Hematology journal/ISSN list
were verified live against NCBI E-utilities on 2026-07-01. The remaining
groups have config entries seeded from this file's mapping but are not yet
independently verified or activated; see `pipeline/config.py` for the
...
- Key secondary endpoints — **explicitly state whether OS was reported/reached**
  when the primary endpoint is a surrogate
- Safety: grade ≥3 AEs of note; treatment-related deaths; discontinuations
- Median follow-up
- Funding source / notable conflicts of interest
- One line: *potential practice relevance* — followed immediately by caveats
